"""资金日历API — 30-day cash flow calendar with repayments and alerts."""
...
def _build_repayment_map(cards: list[Card], today: date, days: int) -> dict[date, list[dict]]:
    ...
    for card in cards:
        ...
        # The repayment_date returned is the next repayment date for existing balance
...
def get_calendar(current_user: UserInfo = Depends(get_current_user_dependency),
                 db=Depends(get_db)):
    """Return 30-day cash flow calendar with settlements, repayments, and alerts."""
    today = date.today()
    days = 30
    ...
    # Build daily calendar with cumulative cash_pool
    cash_pool = Decimal("0")
    daily_entries = []
