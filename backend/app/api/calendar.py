"""资金日历 API backed by the canonical daily cashflow ledger."""
...
def _calendar_entry(day: CashflowDay) -> dict:
...
def get_calendar(
    ...
    """Return a 30-day cash flow calendar."""
    ledger = build_cashflow(db, current_user.id, date.today(), days=30)
    return {"days": [_calendar_entry(day) for day in ledger.days]}
